...
languageDir=D:/LuckiBotROMNew/es
...
do
  ...
  if [[ $file == RobotSystem ]]; then
    ...
    do
        ...
        do
          if [[ $robotSystemFile == $a* ]]; then
            destDir="$orionRobotSystemRootDir/$a/$orionRobotSystemRootDirSuffix/values-es"
            if [ ! -d "$destDir" ]; then
              mkdir $destDir
            fi
            if [[ $robotSystemFile == $a-strings* ]]; then
              echo $robotSystemFile
              mv $languageDir/RobotSystem/$robotSystemFile $orionRobotSystemRootDir/$a/$orionRobotSystemRootDirSuffix/values-es/strings.xml
            elif [[ $robotSystemFile == $a-arrays* ]]; then
              mv $languageDir/RobotSystem/$robotSystemFile $orionRobotSystemRootDir/$a/$orionRobotSystemRootDirSuffix/values-es/arrays.xml
    ...
    do
        ...
        do
          if [[ $robotServiceFile == $a* ]]; then
            destDir="$orionRobotServiceRootDir/$a/$orionRobotServiceRootDirSuffix/values-es"
            if [ ! -d "$destDir" ]; then
              mkdir $destDir
            fi
            if [[ $robotServiceFile == $a-strings* ]]; then
              echo $robotServiceFile
              mv $languageDir/RobotService/$robotServiceFile $orionRobotServiceRootDir/$a/$orionRobotServiceRootDirSuffix/values-es/strings.xml
            fi
          fi
          done
      done
    ...
    do
      destDir="$orionRobotSettingsRootDir/values-es"
      if [ ! -d "$destDir" ]; then
        mkdir $destDir
      fi
      destDir1="$orionRobotSettingsRootDir/xml-es"
      if [ ! -d "$destDir1" ]; then
        mkdir $destDir1
      fi
      if [[ $robotSettingsFile == RobotSettings-values-strings* ]]; then
        echo $robotSettingsFile
        mv $languageDir/RobotSettings/$robotSettingsFile $orionRobotSettingsRootDir/values-es/strings.xml
      elif [[ $robotSettingsFile == RobotSettings-xml-time_zone* ]]; then
        echo $robotSettingsFile
        mv $languageDir/RobotSettings/$robotSettingsFile $orionRobotSettingsRootDir/xml-es/time_zone.xml
      fi
      done
  elif [[ $file == RobotPlatform ]]; then
    for robotPlatformFile in `ls $languageDir/RobotPlatform`;
    do
      destDir="$orionRobotPlatformAppRootDir/values-es"
      if [ ! -d "$destDir" ]; then
        mkdir $destDir
      fi
      destDir1="$orionRobotPlatformRootDir/values-es"
      if [ ! -d "$destDir1" ]; then
        mkdir $destDir1
      fi
      if [[ $robotPlatformFile == RobotPlatform-app-strings* ]]; then
        echo $robotPlatformFile
        mv $languageDir/RobotPlatform/$robotPlatformFile $orionRobotPlatformAppRootDir/values-es/strings.xml
      elif [[ $robotPlatformFile == RobotPlatform-platform-strings* ]]; then
        echo $robotPlatformFile
        mv $languageDir/RobotPlatform/$robotPlatformFile $orionRobotPlatformRootDir/values-es/string.xml
      fi
      done
    ...
    do
        ...
        do
          if [[ $robotPackagesFile == $a.* ]]; then
            ...
            mv $languageDir/RobotPackages/$robotPackagesFile $orionRobotPackagesRootDir/$a/$orionRobotPackagesRootDirSuffix/es.js
          fi
          done
      done
  fi
